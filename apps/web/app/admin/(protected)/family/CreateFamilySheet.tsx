...
export function CreateFamilySheet({ open, onOpenChange, levels, onSave }: CreateFamilySheetProps) {
  ...
  const [highlightedStudentId, setHighlightedStudentId] = React.useState<string | null>(null);
  ...
  const addStudent = () => {
    const nextStudent = createBlankStudent();
    setStudents((prev) => [...prev, nextStudent]);
    setHighlightedStudentId(nextStudent.id);
  ...
  const primaryLabel = step === 1 ? "Next" : submitting ? "Creating..." : "Create family";
  ...
  React.useEffect(() => {
    ...
    const node = scrollAreaRef.current?.querySelector<HTMLElement>(`[data-student-id="${highlightedStudentId}"]`);
    if (node) {
      node.scrollIntoView({ behavior: "smooth", block: "start" });
    }
    const timeout = window.setTimeout(() => setHighlightedStudentId(null), 1200);
                    ...
                    students.map((student, index) => {
                      ...
                      return (
                        <div
                          ...
                          className={cn(
                            "space-y-4 rounded-lg border bg-card/30 p-4 shadow-sm transition",
