...
type BroadcastResult = Awaited<ReturnType<typeof sendBroadcastAction>>;
...
export function ComposeTab({ families, levels, invoiceStatuses }: ComposeTabProps) {
  ...
  const contentLength = (val: string, isEmail: boolean) => {
    if (!isEmail) return val.length;
    const plain = val.replace(/<[^>]+>/g, "").trim();
    return plain.length;
  };

  const hasContent = (val: string, isEmail: boolean) => contentLength(val, isEmail) > 0;
  ...
  const sendBroadcast = () => {
    startBroadcast(async () => {
      setBroadcastStatus(null);
      const res: BroadcastResult = await sendBroadcastAction({
        channel: broadcastChannel,
    ...
    <div className="space-y-8">
      <section className="rounded-lg border bg-card p-4 shadow-sm">
        ...
        <div className="mt-3">
          ...
          {channel === "EMAIL" ? (
            <RichTextEditor value={message} onChange={setMessage} placeholder="Write email content" />
          ) : (
            <Textarea value={message} onChange={(e) => setMessage(e.target.value)} className="mt-1" rows={4} />
          )}
          <div className="mt-1 flex items-center justify-between text-xs text-muted-foreground">
            <span>{contentLength(message, channel === "EMAIL")} characters</span>
            <Button
              type="button"
              onClick={handleDirectSend}
              disabled={!familyId || !hasContent(message, channel === "EMAIL") || directBusy}
            >
      ...
      <section className="rounded-lg border bg-card p-4 shadow-sm">
        ...
        <div className="mt-3">
          <Label className="text-xs">Message</Label>
          {broadcastChannel === "EMAIL" ? (
            <RichTextEditor value={broadcastMessage} onChange={setBroadcastMessage} placeholder="Write email content" />
          ) : (
            <Textarea
          ...
          <div className="mt-1 flex items-center justify-between text-xs text-muted-foreground">
            <span>{contentLength(broadcastMessage, broadcastChannel === "EMAIL")} characters</span>
            <div className="flex gap-2">
              ...
              <Button
                ...
                disabled={!hasContent(broadcastMessage, broadcastChannel === "EMAIL") || broadcastBusy}
              >
