...
export default function TeacherVerifyPage() {
  ...
  React.useEffect(() => {
    const raw = sessionStorage.getItem("caribeae.teacher.auth.pending");
    if (!raw) return;
    try {
  ...
  const handleVerify = React.useCallback(async () => {
    ...
    try {
      ...
      const verifyRes = await fetch("/api/teacher-auth/complete", { method: "POST" });
      const verify = await verifyRes.json().catch(() => null);
      if (!verify?.ok) {
        clearPending();
        await signOut(() => router.replace("/teacher/auth?blocked=1"));
        return;
      }
    ...
    return (
      <AuthShell mode="teacher">
        <div className="space-y-6">
          ...
          <Button className="w-full sm:w-auto" onClick={() => router.push("/teacher/auth")}>
            Back to sign in
          </Button>
        </div>
      </AuthShell>
